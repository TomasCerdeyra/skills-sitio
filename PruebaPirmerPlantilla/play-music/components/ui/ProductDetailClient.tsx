...
export function ProductDetailClient({ product }: { product: Product }) {
  ...
  // Si no hay imágenes, generar placeholder
  const galleryImages =
    sortedImages.length > 0
      ? sortedImages
      : [{ id: "placeholder", url: getProductImage(product), alt: product.name, position: 0 }];
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 lg:px-8 pt-28 pb-20">
      {/* Breadcrumb */}
      <nav className="flex items-center gap-2 font-body text-xs text-neutral-400 mb-10">
        <a href="/" className="hover:text-brand-primary transition-colors">Inicio</a>
        <span>/</span>
        <a href="/catalogo" className="hover:text-brand-primary transition-colors">Instrumentos</a>
        <span>/</span>
      ...
      <div className="grid lg:grid-cols-12 gap-8 lg:gap-16">
        ...
        <div className="lg:col-span-5">
          <div className="lg:sticky lg:top-28">
            ...
            >
              <h1 className="font-display text-3xl lg:text-4xl xl:text-5xl text-neutral-900 font-bold leading-tight mb-5">
                {product.name}
              </h1>

              {/* Price */}
              <div className="flex items-baseline gap-4 mb-8">
                <span className="font-body text-3xl font-bold text-neutral-900">
                ...
                {product.compare_at_price && product.compare_at_price > effectivePrice && (
                  <>
                    ...
                    <span className="font-body text-sm font-bold text-brand-primary bg-brand-primary/10 px-2 py-0.5 rounded-sm">
                      {Math.round(
              ...
              {/* Description */}
              {product.description && (
                <div className="font-body text-base text-neutral-600 leading-relaxed mb-8 whitespace-pre-line">
                  {product.description}
                </div>
              )}
              ...
              <div className="space-y-3 mt-6">
                ...
                <a
                  ...
                  className="w-full flex items-center justify-center gap-2 border-2 border-neutral-800 text-neutral-800 py-3.5 rounded-sm font-body font-medium text-sm hover:bg-neutral-800 hover:text-neutral-50 transition-all duration-200"
                >
                ...
                {!isAvailable && (
                  <p className="font-body text-sm text-neutral-500 text-center">
                    Sin stock disponible en esta variante
                  </p>
                )}
              </div>
              ...
              <div className="mt-8 pt-8 border-t border-neutral-200 space-y-3">
                ...
                {selectedVariant?.sku && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-body text-neutral-700 font-mono text-xs">
                      {selectedVariant.sku}
                    </span>
                  </div>
                )}
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-body text-neutral-700">Hasta 12 cuotas</span>
                </div>
              </div>
            </motion.div>
          </div>

...
Formulario de contacto. Manda un email vía Resend al `CONTACT_EMAIL` configurado. Usa el `resend_api_key` del tenant en Supabase.

**Importante:** si Resend no está configurado, el endpoint igual responde `ok: true` para que el usuario no vea errores internos. Se loggea por consola.

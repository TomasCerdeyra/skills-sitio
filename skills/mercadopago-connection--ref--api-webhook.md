...
Recibe notificaciones asíncronas de MP y actualiza el estado final. Detecta transición de "no-approved" a "approved" para incrementar cupón y enviar email solo una vez.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.type === "payment" && body.data?.id) {
      ...
      // Si pasa a approved por primera vez: aplicar cupón + email
...
- Siempre responde `{ received: true }` para que MP no reintente.
- La detección `!wasApproved && isNowApproved` evita duplicar incremento de cupón y email cuando MP envía múltiples notificaciones para el mismo pago.
- Para testear localmente, usar ngrok y setear `NEXT_PUBLIC_SITE_URL` con la URL de ngrok.

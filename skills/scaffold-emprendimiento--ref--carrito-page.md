# Reference: Página de carrito `/carrito`

Paso intermedio entre el header "Pedido" y el checkout. El usuario puede revisar los productos, ajustar cantidades y eliminar items antes de ingresar sus datos de entrega.

**Path:** `app/(public)/carrito/page.tsx`
...
- **CartContext** debe estar instalado y proveído en `app/layout.tsx` (`<CartProvider>`).
- El botón "Pedido" (o ícono carrito) en `Header` y `MobileMenu` debe enlazar a `/carrito`, NO a `/checkout/datos`.
- La función `updateQuantity(id, qty, variant_id)` elimina el item si `qty <= 0` — el botón "−" con cantidad 1 actúa como eliminación.
- `removeItem(id, variant_id)` elimina sin importar la cantidad.
- `AnimatePresence mode="popLayout"` garantiza la animación de salida al eliminar items.
- La key del `CartItemRow` debe combinar `id` + `variant_id` para distinguir variantes del mismo producto.
